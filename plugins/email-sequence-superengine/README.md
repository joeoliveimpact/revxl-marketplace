...
- A **story-intake** that banks your real stories so the story-driven emails pull from truth, never fabrication.
...
| Skill | Builds | Say |
|-------|--------|-----|
| `precall-nurture` | 4-email strategy-call show-up sequence | "precall sequence", "show-up emails" |
| `precall-video-script` | 5-7 min pre-sell VSL script | "VSL script", "pre-sell video" |
| `launch` | 7-day open-cart launch sequence | "launch sequence", "open cart emails" |
| `warm-nurture` | weekly value-to-invite pattern (1-7/wk) | "warm nurture", "keep my list warm" |
| `no-show-recovery` | 4-touch reschedule sequence | "rebook no-shows", "they didn't show" |
| `post-call-followup` | 4-touch no-close follow-up | "follow up after the call" |
| `winback` | 3-touch win-back / sunset sequence | "winback", "reactivate cold leads" |
| `onboarding` | 5-email 30-day new-client sequence | "onboarding emails", "welcome sequence" |
...
1. Say _"first time"_ or _"walk me through it"_ and the `guide` tour orients you, runs setup, and builds your first sequence
...
| Match your written voice automatically | A workspace voice guide or VoC source | Answer the voice questions in `setup` |
