...
platform reference file is exact (sourced from the pricing registry); trust it over the tier.
...
**Force a live fetch** with the `Cache-Control: no-cache` request header — billed at the normal
rate; the fresh result is written back for the next caller. `no-store` alone does not trigger it.
The cache is shared across the account, so one client's fresh pull benefits the next.
