...
# The Vault trigger points written into each plugin are the primary mechanism;
...
# Vault call happened, and an uncertain nudge is worse than no nudge.
...
# USERPROFILE first: on Windows it is the same folder revxl-vault-search writes
...
# command that made the call (revxl-vault-search step 3), so it is always UTC.
sub iso_epoch {
    my ($s) = @_;
...
# A failed Vault call is still a Vault check: revxl-vault-search logs one line
