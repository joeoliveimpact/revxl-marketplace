...
| **Closeout did not complete** | Say which phase stopped and what that costs, then offer the choice. *"Closeout stopped before it rewrote handoff.md, so anything I put on a chip would be yesterday's plan. Want me to finish the closeout first, or spawn a chip that says the handoff is stale?"* Never build silently on a stale handoff. **If no user can answer** (unattended run, subagent, scheduled context), the default is the first option: attempt to finish the closeout — resume it per Step 1's resume rules — and re-check the receipt. If that also fails, stop, write nothing, and leave a one-paragraph note of exactly where it died for the next attended session. A dead-end with a readable note beats either guess: a silent second closeout splits the record, and a stale-marked chip spawns a session nobody asked for. This default exists because a live test dead-ended here — the branch resolved only by a user choice no unattended context could make. |
