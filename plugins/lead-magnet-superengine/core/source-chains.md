...
| **Video / webinar / VSL teardown** — transcript + visual breakdown of a competitor video | yt-dlp (pull) → ffmpeg (audio extract / frame grab) → Whisper server (endpoint from profile `transcribe` block) → Groq (if `GROQ_API_KEY` present, model `whisper-large-v3-turbo`, **with a vocabulary `prompt` built from the brand's canonical names** — see `lm-setup` → Transcribe) → `video-use` skill (visual scene analysis) | `"Paste the transcript of the video, or describe the key points you remember from it."` |
