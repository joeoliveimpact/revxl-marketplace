...
REEL_SCRIPTER='write a reel script|script a reel|reel script|reel-scripter|script the next reel'
...
if echo "$input" | grep -qiE "$COMPASS"; then
  nudge shortform-next
elif echo "$input" | grep -qiE "$START"; then
