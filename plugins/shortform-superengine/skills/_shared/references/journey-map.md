...
| F6 | any skill, BOTH `analysis.date` and `pulse.last_run` older than 30 days (a null `pulse.last_run` counts as over 30 days), or `analysis.date` alone older than 90 days, which the pulse never refreshes | 30 to 90 days: refresh the field via the pulse as move #1, with the staleness named. `analysis.date` 90 days or older: a full competitor-cross-reference re-run instead, with its `N x 3` credit cost stated |
