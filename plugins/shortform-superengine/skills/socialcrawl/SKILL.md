...
description: >
  ...
  crawl", or "social media API". socialcrawl-superengine, when installed,
  executes the searches; the method and the guidelines stay here.
---
...
**Next moves ... no key**
(E0) no SocialCrawl key resolved, so no call was made and nothing was spent.
1. Run setup: it wires the key, saves it for future sessions, and audits the rest. Say: "set up shortform superengine"
2. Not sure where you are in the flow? Say: "what's next in shortform"

**Next moves ... credits short**
(F4) the balance will not cover this call. Never spend silently.
1. Read the balance first, it costs 0 credits. Say: "check my SocialCrawl balance"
2. Cheaper answer to the same question: batch stats for up to 100 post URLs at 1 credit (see Cheat codes). Say: "batch-check engagement"
3. Top up, then pick the move worth the credits. Say: "what's next in shortform"
