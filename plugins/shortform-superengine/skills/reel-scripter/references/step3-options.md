...
Moved out of `SKILL.md` at 0.4.0 for the byte ceiling; the doctrine is unchanged. Step 3
...
1. **Generate** options in the client's voice, each built on the brief's proven moves:
   - **Hook (5–8 options)** — obey `./hook-mastery.md` law (single subject, single
   ...
   - **Secondary hooks — compute placements FIRST (mandatory), then 3 options per placement.**
     ...
     `./retention-psychology.md` §4's dosage table (30s→1 · 45s→1–2 · 60s→2–3 ·
   ...
   - **CTA (2–3 options)** — matching scaffold from `./cta-scaffolds.md`, mapped to
...
The three-hook alignment check and the 8 Swaps line-edit pass stay in `SKILL.md` Step 3;
the swaps themselves are the table in `./story-locks.md`.