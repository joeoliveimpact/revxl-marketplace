...
w('> How to use: Sections 1-3 tell you WHAT to make (themes + hook types that overperform). '
  ...
  'what you do now and those winning moves. Section 8 is the do-not list -- this niche\'s own '
  'losers. Pick a Section-1 theme, a Section-2 hook, an opener from Section 4 -> that is one reel.')
if not FULL:
    w('')
...
# ---------------------------------------------------------------------------
# 8. AVOID -- this niche's losers (niche-first firewall; guarded so noise never bans)
# ---------------------------------------------------------------------------
w('')
...
flop_only = []
if FULL:
    # words that cluster in flop openings and NEVER in winner openings = sharpest avoid vocab
    _winner_words = {y['word'] for y in jx['openers'].get('winner_vocab', [])}
    flop_only = [x for x in jx['openers'].get('flop_vocab', []) if x['word'] not in _winner_words]
    if flop_only:
        w('**Opening vocabulary that clusters in FLOPS only (avoid in your first line):** '
          + ', '.join(f"{x['word']}({x['count']})" for x in flop_only))
    else:
        w('**Opening vocabulary:** no words cluster in flops without also appearing in winners -- '
          'no confident vocabulary avoid.')
else:
    w('> Avoid menu limited: no transcripts (structural losers only). Harvest transcripts to add '
...
jx['avoid'] = {
    ...
    'loser_themes': [{'theme': t['theme'], 'field_med_views': t['field_med_views'],
                      'field_reels': t['field_reels']} for t in loser_themes],
    'flop_only_vocab': flop_only,
}
