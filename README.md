...
18 plugins. Versions are current as of catalog `0.1.69`.
...
| [socialcrawl-superengine](plugins/socialcrawl-superengine/) | 0.2.2 | Social research plays on the SocialCrawl API: competitor content, trends, engagement. 3 skills. |
...
| [booking-bandit](plugins/booking-bandit/) | 0.0.1 | Drafts Instagram DM replies in your own voice so more conversations turn into booked calls. Skeleton release: catalog entry and plugin structure only, no drafting logic yet. 1 placeholder skill. |
...
| [workspace-superengine](plugins/workspace-superengine/) | 0.15.1 | Workspace lifecycle: scaffold, session-start, session-closeout, checkpointing, cleanup. Plus `/update-everything`, the one command that updates every plugin, marketplace, and skill layer at once, and `revxl-vault-search`, the one way RevXL plugins reach the RevXL Vault. Stops Claude from forgetting where you left off. 15 skills + 1 agent. |
...
| [course-crawler](plugins/course-crawler/) | 0.7.1 | Capture online course content into structured, searchable local knowledge. 5 skills. |
